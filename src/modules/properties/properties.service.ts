...
export class PropertiesService {
    ...
    if (format === 'card') {
      ...
      //  .leftJoinAndSelect('units.images', 'unitImages')
        ...
        .select([
          ...
          'property.development_delivery_date',
          'property.sub_location_id',
          'property.neighborhood_id',
          'property.sub_neighborhood_id',
  ...
  async findByTokkoPublicationId(publication_id: string): Promise<Property> {
    ...
    if (!property) {
      throw new NotFoundException(`Propiedad con Publication ID ${publication_id} no encontrada`);
    }
  ...
  //  .leftJoinAndSelect('units.images', 'unitImages')
    ...
    return props.map((p) => ({
      ...
      units: p.units ? p.units.map((unit) => ({
        ...
        total_surface: unit.total_surface,
        room_amount: unit.room_amount,
        suite_amount: unit.suite_amount,
  ...
  private appendPropertyOrder(
    ...
    qb
      .addSelect('COALESCE(p.visibility, 0)', 'visibility_sort')
      .orderBy('visibility_sort', 'DESC');
